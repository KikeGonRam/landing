...
export function Footer() {
  ...
  return (
    <footer
      ref={sectionRef}
      className="relative pt-20 pb-12 px-8 lg:px-16 bg-theme-primary overflow-hidden transition-colors duration-300"
    >
      {/* Marquee section */}
      <div
        ref={marqueeRef}
      ...
      >
        {/* Gradient masks */}
        <div className="absolute left-0 top-0 bottom-0 w-40 bg-gradient-to-r from-theme-primary to-transparent z-10" />
        <div className="absolute right-0 top-0 bottom-0 w-40 bg-gradient-to-l from-theme-primary to-transparent z-10" />
          ...
          <div className="marquee-content flex items-center gap-8 text-[56px] lg:text-[112px] font-medium whitespace-nowrap">
            {[...Array(4)].map((_, i) => (
              <span key={i} className="flex items-center gap-8">
                ...
                <span className="text-theme-primary/30 mx-4">&bull;</span>
              </span>
            ))}
          </div>
        </div>
      ...
      <div
        ref={borderRef}
        className="h-px bg-theme-primary/20 mb-16"
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          ...
          <div className="space-y-4">
            {footerConfig.navLinks1.map((link, i) => (
              <a
                ...
                href={link.href}
                className="block text-body text-theme-muted hover:text-theme-primary transition-colors duration-300 group relative overflow-hidden"
              >
                <span className="block group-hover:-translate-y-full transition-transform duration-400">
                  {link.label}
                </span>
                <span className="absolute top-full left-0 block group-hover:-translate-y-full transition-transform duration-400 text-theme-primary">
                  {link.label}
                </span>
              </a>
            ))}
            ...
            {footerConfig.navLinks2.map((link, i) => {
              ...
              return (
                <a
                  ...
                  href={link.href}
                  className="flex items-center gap-2 text-body text-theme-muted hover:text-theme-primary transition-colors duration-300 group"
                >
                  {IconComponent && (
          ...
          <div className="col-span-2 lg:text-right">
            <a
              ...
              className="inline-flex items-center gap-3 text-h5 lg:text-h4 text-theme-primary font-medium group hover:text-highlight transition-colors duration-300"
            >
              {footerConfig.ctaText}
              <span className="w-10 h-10 rounded-full border border-theme-primary/30 flex items-center justify-center group-hover:border-highlight group-hover:shadow-[0_0_15px_var(--highlight)] transition-all duration-300">
        ...
        <div
          ref={copyrightRef}
          className="mt-20 pt-8 border-t border-theme-primary/10 flex flex-col lg:flex-row justify-between items-center gap-4"
        >
          <p className="text-body-sm text-theme-muted">
            {footerConfig.copyright}
          </p>
          <p className="text-body-sm text-theme-muted/70">
